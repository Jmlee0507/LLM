...
def register_user(user: RegisterRequest, db:Session=Depends(get_db)):
    ...
    # 같은 사용자가 있으면(=이름이 같으면) 400에러로 응답
    # but DB에서는 이름과 이메일 둘다 unique라고 해둠
    # 이름만 다르게하고 이메일이 같으면, DB쪽 unique 어쩌구로 회원가입 실패라고 뜬다
    if existing_user:
...
def login(user:UserCreate, db:Session=Depends(get_db)):
    ...
    print(user)
    found =  db.query(User) \
        .filter(User.username == user.username or User.password == user.password) \
        .first()
